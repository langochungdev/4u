...
export const getTemplateConfig = async (templateName: string): Promise<TemplateConfig | null> => {
    try {
        ...
        const configs = import.meta.glob('@/pages/templates/**/config.ts', { eager: true }) as Record<string, any>;

        // Nếu có dấu /, coi như đường dẫn đầy đủ
        if (templateName.includes('/')) {
            // Thử nhiều format path khác nhau
            const possiblePaths = [
                `/src/pages/templates/${templateName}/config.ts`,
                `/@/pages/templates/${templateName}/config.ts`,
                `/pages/templates/${templateName}/config.ts`,
            ];
